...
void GetAligns( const int K, const String& fastb_file, const String& lookup_file, 
  ...
  if ( !USE_CACHE || !IsRegularFile(aligns_file) ) {    
    ...
    for ( int j = 0; j < nproc; j++ ){    
      SystemSucceed( "QueryLookupTable K=12 MM=12 MC=0.3 MF=5000 MAX_ERROR_PERCENT=5 " 
		     + ARG(SEQS, fastb_file) 
...
int main( int argc, char *argv[] ){
  ...
  if ( ! IsRegularFile(inversions_file) || REBUILD_QUERIES ){
    ...
    map< int, map< double, pair<int,vec<alignlet> > > > loci;  
	...
	for ( size_t j = 0; j < raligns.size(); j++ ){
	  ...
	  if ( la1.TargetId() == la2.TargetId() && la1.Fw1() == !la2.Fw1() ){
	    ...
	    double center = q0 <= t0 ? q0 + (double)dist/2.0 : t0 + (double)dist/2.0;
	    if ( abs(dist) < WINDOW ){
    ...
    for ( map<int,map<double,pair<int,vec<alignlet> > > >::iterator i = loci.begin(); i != loci.end(); i++ ){
      int tid = i->first;
      size_t tlen = aseqs[tid].size();
      vec<ho_interval> intervs;
      for ( map<double,pair<int,vec<alignlet> > >::iterator j = i->second.begin(); j != i->second.end(); j++ ){
	if ( j->second.first  > 0 ){
	...
	for ( size_t ic = 0; ic < cov.size(); ic++ ){
	  if ( abs(cov[ic].Stop() - cov[ic].Start()) > K ){
	    basevector invseq( aseqs[tid], cov[ic].Start(), cov[ic].Stop() - cov[ic].Start() );
	    basevector invseq_rc = invseq; invseq_rc.ReverseComplement();
	    if ( invseq == invseq_rc ){
	      basevector newseq;
	      int len = invseq.size() / 2;
	      newseq.SetToSubOf( invseq, 0, len );
	      invseq = newseq;
	    }

	    invseqs.push_back( invseq );
	    invseqs.push_back( invseq.ReverseComplement() );
	  }
	}		
	vec<int> poss;
  ...
  cout << Date() << ": aligning " << invseqs.size() << " candidate inversions to assembly" << endl;
  ...
  if ( REF.nonempty() ){
    ...
    ofstream iout( (work_dir + "/" + assembly_head + ".invers.wrong.locs_on_scaffolds").c_str() );
    ofstream rout( (work_dir + "/" + assembly_head + ".invers.wrong.locs_on_ref").c_str() );
    for ( size_t qid = 0; qid < ia1_index.size(); qid++ ){
      if ( isWrongInver[qid] && ia1_index[qid].size() == 2 ){
	look_align& la1 = inversions_aligns1.at( ia1_index[qid][0] );
	look_align& la2 = inversions_aligns1.at( ia1_index[qid][1] );
	iout << la1.TargetId() << "\t" << la1.StartOnTarget() << "\t" << la1.EndOnTarget() << "\t";
	iout << la2.TargetId() << "\t" << la2.StartOnTarget() << "\t" << la2.EndOnTarget() << "\n";
	if ( ia2_index.at(qid).size() == 1 ){
	  look_align& la = inversions_aligns2.at( ia2_index[qid][0] );
	  rout << la.TargetId() << "\t" << la.StartOnTarget() << "\t" << la.EndOnTarget() << "\n";
	}else
	  rout << "\n";
