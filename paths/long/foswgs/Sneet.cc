...
// This can be run in parallel.  It only generates files that depend on N.
...
int main(int argc, char *argv[])
{
     ...
     CommandArgument_UnsignedInt_OrDefault_Doc(SWAK,0,"if SWAK>0, then new aligner is run with K=SWAK");
     CommandArgument_Int_OrDefault_Doc(VERB,0,"verbosity level for EvalAssembly");
     EndCommandArguments;
     uint NUM_THREADS = 0;
     ...
     BinaryReader::readFile( "/wga/dev/jaffe/BroadCRD/delta.shbv", &shb );
     ...
     for ( int g = 0; g < (int) G.size( ); g++ )
     {
          ...
          Ofstream( out, "blork." + ToString( fos[g] ));
          AssemblyError err = EvalAssembly( ref, shb2, inv2, out, SWAK, VERB );
          #pragma omp critical
          {    gaps.append(err.GetGaps()), indels.append(err.GetIndels());
               subs += err.GetSubs();
               vec<int> lgaps = err.GetLeftGaps( ), rgaps = err.GetRightGaps( );
               endgaps.append(lgaps);
               endgaps.append(rgaps);
               cout << "done with " << fos[g] << endl;    }    }
     ReverseSort(gaps), ReverseSort(indels);
     ReverseSort(endgaps);
     cout << "gaps = {" << printSeq(gaps) << "}, endgaps = {"
          << printSeq(endgaps) << "}, indels = {" << printSeq(indels)
          << "}, subs = " << subs << endl;    }
