...
my $cmd = "java -jar $gatk/GenomeAnalysisTK.jar -T LiftoverVariants -R $oldRef.fasta -V:variant $in -o $unsorted_vcf -chain $chain -dict $newRef.dict";
...
$cmd = "grep \"^#\" -v $unsorted_vcf | sort -n -k2 -T $tmp | $gatk/sortByRef.pl --tmp $tmp - $newRef.fasta.fai >> $sorted_vcf";
...
$cmd = "java -jar $gatk/GenomeAnalysisTK.jar -T FilterLiftedVariants -R $newRef.fasta -V:variant $sorted_vcf -o $out";
