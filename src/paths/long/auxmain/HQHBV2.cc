...
// HQHBV2.  Like HQHBV, but has a separate threshold MIN_FREQ2 for kmers present
...
{    double clock = WallClockTime( );
     ...
     CommandArgument_Int_OrDefault_Doc(MIN_FREQ2, 3, "minimum kmer frequency if fw/rc");
          ...
          {    vec<int64_t> this_kfreq( 100, 0 );
               ...
               {    int64_t j = kmers.NextDiff(i);
                    ...
                    {    int f = 0, r = 0;
                         ...
                         if ( f > 0 && r > 0 ) this_passing[p].push_back( kmers[i] );    }
                    i = j - 1;    }    }
