...
int main(int argc, char *argv[])
{
     ...
     {    if ( ids[i] < 0 )
          {    cout << "\nRead id " << ids[i] << " is negative." << endl;
               Scram(1);    }
          if ( ids[i] >= nreads )
     ...
     {    cout << "\nscrutinizing placements:\n";
          ...
          {    const ReadPath &p1 = p[2*i], &p2 = p[2*i+1];
               ...
               {    if ( x1.empty( ) ) 
                    {    out << "p1 unplaced" << endl;
                         problems = True;    }
                    else if ( p1.getOffset( ) < 0 )
                    {    out << "p1 has negative start" << endl;
                         problems = True;    }
                    else
                    {    int start = p1.getOffset( );
                         out << start << ":" << printSeq(p1) << endl;
                         int e = p1[0];
