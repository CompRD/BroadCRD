...
/**
 * SplitFastbIntoFastaFiles
 *
 * Split a fastb file into fixed-size fasta files. See also its twin
 * module, SplitQualbIntoQualFiles.
 *
 * RUN_DIR: where the fastb file is
 * OUT_DIR: as a subdir of RUN_DIR
 * HEAD: head of fastb and ids file
 * SELECT: use these ids only (full path name, parsed with ParseIntSet)
 * BATCH_SIZE: how many reads in each fasta file
 * NAMES: load ids file and use read names in the fasta output
...
int main( int argc, char *argv[] )
{
  ...
  CommandArgument_String_OrDefault( OUT_DIR, "Reads.fasta" );
  ...
  // Dir and file names.
  String bases_file = RUN_DIR + "/" + HEAD + ".fastb";
  ...
  if ( IsDirectory( out_dir ) ) {
    if ( OVERWRITE ) {
      cout << Date( ) << ": removing fastas from old run" << endl;
      String comm = "rm -rf " + out_dir + "/*.fasta";
      System( comm );
    }
    else {
  ...
  int tot_count = MastervecFileObjectCount( bases_file );
  ...
  for (int batch_id=0; batch_id<n_batches; batch_id++) {
    ...
    vecbasevector bases;
    bases.SparseRead( bases_file, read_ids, 0 );
    
    String out_file = out_base + ToString( batch_id ) + ".fasta";
    ofstream out( out_file.c_str( ) );
    for (int ii=begin_id; ii<end_id; ii++) {
      int rid = sel[ii];
      String read_name = NAMES ? ids[rid] : "read_" + ToString( rid );
      bases[rid].Print( out, read_name );
    }
    out.close( );
  }
