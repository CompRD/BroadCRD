...
class digraph
{
     ...
     void DOT_vl( ostream& out, const vec<String> & vertex_labels,
          ...
          const vec<String>& vertex_colors = vec<String>( ) ) const;
...
template<class F> class digraphE : public digraph {
     ...
     enum ConstructorType1 {
          COMPLETE_SUBGRAPH,
     };

     enum ConstructorType2 {
     ...
     // Constructor 5b: given a digraph, and given a unique-sorted list of edge 
     // indices, create the digraph having just those edges (in the given order), 
     ...
     // (Note buggy handling if there are cycles at w: these will not be seen.)
     // Call the second version unless you want your code to be extremely slow!!
     ...
     Bool EdgePaths( const vec<int>& to_left, const vec<int>& to_right, const int v,
          ...
          const int max_paths = -1, const int max_iterations = -1 ) const;
