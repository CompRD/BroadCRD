...
// SmithWatBandedQual( S, T, SQ, offset, bandwidth )
//
// (need to edit to explain use of SQ)
//
...
template<class X>
float SmithWatBandedQual2( const basevector& S, const basevector& T, 
     const qualvector& SQ, int offset, int bandwidth, align& a, int& errors, 
     ostream *log, int MIS, int INS, int DEL)
{
	 ...
	 out << "Warning: SmithWatBandedQual passed nonsense arguments:" << endl;
     ...
     {    x[0] = 0;
          ...
          #define SWCORE(J)                                                        \
               ...
               {    if ( istart <= istop )                                         \
                    {    int a = lastx + mismatch_penalty * SQ[istart] * (sp[istart] != J);     \
                         int b = *xp + ins_penalty * SQ[istart];                   \
                         ++xp;                                                     \
                         int c = *xp + del_penalty * SQ[istart];                   \
                         ...
                         SWMIN(istart)    }                                        \
                    if ( istart + 1 <= istop )                                     \
                    {    int a = lastx + mismatch_penalty * SQ[istart+1] * (sp[istart+1] != J);   \
                         int b = *xp + ins_penalty * SQ[istart+1];                 \
                         ++xp;                                                     \
                         int c = *xp + del_penalty * SQ[istart+1];                 \
                         if ( !( left <= istart - (j-1)                            \
                    ...
                    {    int a = lastx + mismatch_penalty * SQ[i] * (sp[i] != J);          \
                         int b = *xp + ins_penalty * SQ[i];                        \
                         ++xp;                                                     \
                         int c = *xp + del_penalty * SQ[i];                        \
                         SWMIN(i)    }                                             \
                    if ( istop - 1 >= istart + 2 )                                 \
                    {    int a = lastx + mismatch_penalty * SQ[istop-1] * (sp[istop-1] != J);    \
                         int b = *xp + ins_penalty * SQ[istop-1];                  \
                         ++xp;                                                     \
                         int c = *xp + del_penalty * SQ[istop-1];                  \
                         if ( !( istop - j <= right ) ) b = Infinity;              \
                         SWMIN(istop-1)    }                                       \
                    if ( istop >= istart + 2 )                                     \
                    {    int a = lastx + mismatch_penalty * SQ[istop] * (sp[istop] != J);      \
                         int b = *xp + ins_penalty * SQ[istop];                    \
                         ++xp;                                                     \
                         int c = *xp + del_penalty * SQ[istop];                    \
     ...
     {    
          ...
          /*
          cout << "Warning: SmithWatBandedQual produced a garbage alignment, "
...
template float SmithWatBandedQual2<unsigned char>( const basevector&, 
     const basevector&, const qualvector&, int, int, align&, int&, ostream*, int, 
     int, int );
template float SmithWatBandedQual2<unsigned short>( const basevector&, 
     const basevector&, const qualvector&, int, int, align&, int&, ostream*, int, 
     int, int );
template float SmithWatBandedQual2<unsigned int>( const basevector&, 
     const basevector&, const qualvector&, int, int, align&, int&, ostream*, int, 
     int, int );
